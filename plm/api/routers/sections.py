...
from ..deps import get_db, get_project, get_settings, get_store, raise_service, require_project
...
def create_sections(project_id: str, body: SectionParams, background: BackgroundTasks, request: Request, p: dict = Depends(get_project),
    ...
    job = start_job("sections", params, project_id, sync, background, db, settings, user, request)
    if job["status"] == "error":
...
def list_sets(project_id: str, alignment_id: int | None = None, p: dict = Depends(get_project), store: ProjectStore = Depends(get_store), _: dict = Depends(require_project("viewer"))):
    return [SectionSetOut(**s) for s in store.section_sets(alignment_id)]


@router.get("/sections/{set_id}", response_model=SectionSetOut)
def get_set(project_id: str, set_id: int, full: bool = True, p: dict = Depends(get_project), store: ProjectStore = Depends(get_store), _: dict = Depends(require_project("viewer"))):
...
def delete_set(project_id: str, set_id: int, p: dict = Depends(get_project), store: ProjectStore = Depends(get_store), _: dict = Depends(require_project("editor"))):
...
def profile_csv(project_id: str, set_id: int, p: dict = Depends(get_project), store: ProjectStore = Depends(get_store), _: dict = Depends(require_project("viewer"))):
    try:
...
def cross_csv(project_id: str, set_id: int, p: dict = Depends(get_project), store: ProjectStore = Depends(get_store), _: dict = Depends(require_project("viewer"))):
    try:
...
def lines_geojson(project_id: str, set_id: int, crs: str | None = None, p: dict = Depends(get_project), store: ProjectStore = Depends(get_store), _: dict = Depends(require_project("viewer"))):
    try:
