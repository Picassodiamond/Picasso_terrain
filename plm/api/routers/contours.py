...
def create_contours(project_id: str, body: ContourParams, background: BackgroundTasks, request: Request, p: dict = Depends(get_project),
    ...
    params["run_id"] = rid
    job = start_job("contours", params, project_id, sync, background, db, settings, user, request)
    if job["status"] == "error":
...
def list_sets(project_id: str, run_id: int | None = None, p: dict = Depends(get_project), store: ProjectStore = Depends(get_store), _: dict = Depends(require_project("viewer"))):
...
def get_set(project_id: str, set_id: int, p: dict = Depends(get_project), store: ProjectStore = Depends(get_store), _: dict = Depends(require_project("viewer"))):
...
def patch_set(project_id: str, set_id: int, body: ContourSetPatch, p: dict = Depends(get_project), store: ProjectStore = Depends(get_store), _: dict = Depends(require_project("editor"))):
    if store.get_contour_set(set_id) is None:
        raise HTTPException(status_code=404, detail="contour set not found")
